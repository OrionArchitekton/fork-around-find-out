...
HTML — no client round-trip races the capture.
...
**Narration:** "Instead of running a risky action, we fork the live Daytona sandbox, run the action in the fork, and measure exactly what it did — before the real world sees it."

### Shot 3 — A benign action ALLOWs (0:36–0:54)
**Goto:** `/?demo=benign-build`
**On screen:** green ALLOW verdict + blast radius (files created, zero secrets, zero egress) + "merged to real world"
**Narration:** "A normal action — running the tests — has a clean blast radius, passes policy, and merges to the real world."
...
**Narration:** "Now a prompt-injected action tries to read the .env secret and beacon it to an attacker. In the fork, we watch it happen — secret read, network egress — the exfiltration shape. Blocked. The real world was never touched."

### Shot 5 — The catch-rate, scored (1:16–1:38)
**Goto:** `/` then click "Run full suite → Braintrust" (or `/?suite=1` prerender)
**On screen:** big "100%" harmful caught, per-scenario ALLOW/BLOCK ticks
**Narration:** "Every verdict is scored against a golden attack dataset in Braintrust — a real catch-rate, not a vibe. A hundred percent of the attacks caught, every benign action passed."
...
- The recorded demo runs in **mock mode** — say "the world is a Daytona fork" (true of the
  ...
  this note.
- "100% caught" is over the **6-scenario golden suite** — say "every attack in our suite,"
